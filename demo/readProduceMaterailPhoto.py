...
def recognize(imgCut,knn,idx=1):
    ...
    cv.imwrite(fpath+'readProduceMaterial/dst.jpg',dst)
    ...
    if len(contoursOrderd)>0:
        ...
        for i2,c in enumerate(contoursOrderd):
            ...
            if(w>30 and w<45 ):
                ...
                #cv.imwrite('G:/project/photo-MES/demo/p/p'+str(i2)+'a.jpg',mask)

                num3 = np.array(mask);
                num3_ = num3.reshape(-1,20*40).astype(np.float32)
                ret, result, neighbours ,dist = knn.findNearest(num3_,k=5)
                results.append(result[0][0])
                cv.imwrite(fpath+'readProduceMaterial/knn/'+str(idx)+'_'+str(i2)+' '+str(result[0][0])+'a.jpg',mask)
                
                mask = np.zeros((40, 20), dtype=roi.dtype)
                mask[int((40-h2)/2):int((40-h2)/2)+h2, int((20-w2)/2):int((20-w2)/2)+w2] = rp2[:h2,:w2]
                #cv.imwrite('G:/project/photo-MES/demo/p/p'+str(i2)+'b.jpg',mask)  
                num3 = np.array(mask);
                num3_ = num3.reshape(-1,20*40).astype(np.float32)
                ret, result, neighbours ,dist = knn.findNearest(num3_,k=5)
                results.append(result[0][0])           
                cv.imwrite(fpath+'readProduceMaterial/knn/'+str(idx)+'_'+str(i2)+' '+str(result[0][0])+'b.jpg',mask)
            else:    
                if(w>20):w=20
                mask = np.zeros((40, 20), dtype=roi.dtype)
                ...
                ret, result, neighbours ,dist = knn.findNearest(num3_,k=5)
                results.append(result[0][0])
                cv.imwrite(fpath+'readProduceMaterial/knn/'+str(idx)+'_'+str(i2)+' '+str(result[0][0])+'.jpg',mask)
    rt = ''
...
def readPhotoAndRotate(img):
    ...
    cv.imwrite(fpath+'readProduceMaterial/img_original.jpg',img_original)
    #AB线性锐化,使得4个mark点突出
    ...
    cv.imwrite(fpath+'readProduceMaterial/res.jpg',res)
    ...
    for pt in zip(*loc[::-1]):
        ...
        if(containd==0):
            ...
            cv.circle(img,(ptt[0],ptt[1]),2,(0,0,255),1)
            c[0]=int(pt[0] + w/2)+c[0]
            c[1]=int(pt[1] + h/2)+c[1]
    c[0]=int(c[0]/4)
    c[1]=int(c[1]/4)
    cv.circle(img,(c[0],c[1]),4,(0,0,255),1)
    ...
    for pt in zip(*loc[::-1]):
        ...
        if(i[0]>c[0] and i[1]>c[1]):
            rb[0]=int(i[0])
            rb[1]=int(i[1])        
    cv.line(img,(lt[0],lt[1]),(lb[0],lb[1]),(0,0,255),1)
    cv.line(img,(rt[0],rt[1]),(rb[0],rb[1]),(0,0,255),1)
    ...
    cv.imwrite(fpath+'readProduceMaterial/template.jpg',img)
...
if __name__ == "__main__":
    ...
    #arg_areas = '154,1855,180,1381 ,"box", 260,343,374,777 ,"time_start" , 260,321,1208,1339 ,"user1"'
    ...
    for i1,c in enumerate(areas):
        ...
        if name == 'box':
            ...
            cv.imwrite(fpath+'readProduceMaterial/rotated.jpg',rotated)
        else:
            ...
            cv.imwrite(fpath+'readProduceMaterial/area1_img'+str(i1)+'_'+data+'.jpg',area1_img)
